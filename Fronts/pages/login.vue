  ...
  <div class="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl p-6">
      <div class="text-center mb-8">
        <h1 class="text-3xl font-bold text-gray-800">Login Razoável</h1>
        <p class="mt-2 text-gray-600">Segurança no front</p>
      </div>
...
function validateForm() {
  ...
  if (!loginRegex.test(formData.value.login)) {
    return 'O login deve conter de 3 a 20 caracteres alfanuméricos ou _.'
  }

  if (!senhaRegex.test(formData.value.senha)) {
    return 'A senha deve conter pelo menos 6 caracteres e pode incluir símbolos.'
  }
